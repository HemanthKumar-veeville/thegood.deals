/*
We're constantly improving the code you see. 
Please share your feedback here: https://form.asana.com/?k=uvp-HPgd3_hyoXRBw1IcNg&d=1152665201300829
*/

import React from "react";

export const CheckmarkCircle1 = ({ className }) => {
  return (
    <svg
      className={`${className}`}
      fill="none"
      height="24"
      viewBox="0 0 24 24"
      width="24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_310_6722)">
        <path
          d="M12 0.675049C5.73755 0.675049 0.675049 5.73755 0.675049 12C0.675049 18.2626 5.73755 23.3625 12 23.3625C18.2626 23.3625 23.3625 18.2626 23.3625 12C23.3625 5.73755 18.2626 0.675049 12 0.675049ZM12 21.675C6.67505 21.675 2.36255 17.325 2.36255 12C2.36255 6.67505 6.67505 2.36255 12 2.36255C17.325 2.36255 21.675 6.71255 21.675 12.0375C21.675 17.325 17.325 21.675 12 21.675Z"
          fill="#111928"
        />
        <path
          d="M15.2251 8.51251L10.7626 12.8625L8.73761 10.875C8.40011 10.5375 7.87511 10.575 7.53761 10.875C7.20011 11.2125 7.23761 11.7375 7.53761 12.075L9.93761 14.4C10.1626 14.625 10.4626 14.7375 10.7626 14.7375C11.0626 14.7375 11.3626 14.625 11.5876 14.4L16.4251 9.75001C16.7626 9.41251 16.7626 8.88751 16.4251 8.55001C16.0876 8.21251 15.5626 8.21251 15.2251 8.51251Z"
          fill="#111928"
        />
      </g>
      <defs>
        <clipPath id="clip0_310_6722">
          <rect fill="white" height="24" width="24" />
        </clipPath>
      </defs>
    </svg>
  );
};
